...
data class JoinLink(
    ...
    fun encode(): String = buildString {
        ...
        require(it.length <= MAXIMUM_JOIN_LINK_BYTES) { "ADC join link is too long" }
    }

    fun displayFingerprint(): String = signerFingerprint.chunked(4).joinToString(" ")

    companion object {
        private const val PREFIX = "adc://join/v1?"
        ...
        fun parse(encoded: String): JoinLink {
            require(encoded.length <= MAXIMUM_JOIN_LINK_BYTES && encoded.startsWith(PREFIX)) {
                "Invalid ADC join link"
            }
            val rawParts = encoded.removePrefix(PREFIX).split('&')
            require(rawParts.size == QUERY_KEYS.size) { "Invalid ADC join query" }
            val values = rawParts.mapIndexed { index, part ->
                val separator = part.indexOf('=')
                require(separator > 0 && part.indexOf('=', separator + 1) < 0) { "Invalid ADC join query" }
                require(part.substring(0, separator) == QUERY_KEYS[index]) { "Invalid ADC join query" }
            ...
            ).also { require(it.encode() == encoded) { "ADC join link is not canonical" } }
        }
        ...
        private fun percentDecode(value: String): String {
            ...
            while (index < value.length) {
                ...
                when {
                    character == '%' -> {
                        require(index + 2 < value.length) { "Invalid ADC join escaping" }
                        val high = value[index + 1].digitToIntOrNull(16)
                        val low = value[index + 2].digitToIntOrNull(16)
                        require(high != null && low != null) { "Invalid ADC join escaping" }
                    ...
                    else -> throw IllegalArgumentException("ADC join query must be ASCII")
                }
            }
