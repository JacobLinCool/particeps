...
class JoinLinkTest {
    ...
    fun parserRejectsMutableOrAmbiguousJoinLinks() {
        val valid = JoinLink(
            URI("https://artifacts.example.invalid/config.adccfg"),
            "0".repeat(64),
            "A".repeat(32),
        ).encode()
        val encodedArtifact = "https%3A%2F%2Fartifacts.example.invalid%2Fconfig.adccfg"
        ...
        listOf(
            ...
            valid.replace(
                encodedArtifact,
                "https%3A%2F%2Fuser%40artifacts.example.invalid%2Fconfig.adccfg",
            ),
    ...
    fun encoderRejectsAnArtifactWhoseEscapedJoinLinkExceedsTheWireBound() {
        val link = JoinLink(
            URI("https://artifacts.example.invalid/${"a/".repeat(1_000)}config.adccfg"),
            "0".repeat(64),
            "A".repeat(32),
        )
