...
describe("Particeps receiver in workerd", () => {
  it("stores and exactly replays the shared Protocol v1 bundle through native R2", async () => {
    ...
    expect(metadata!).toMatchObject({
      ...
      first_commit_sequence: receipt.first_commit_sequence,
      last_commit_sequence: receipt.last_commit_sequence,
...
function requestInit(
  ...
  return {
    ...
    headers: {
      ...
      "X-Particeps-Commit-From": receipt.first_commit_sequence,
      "X-Particeps-Commit-To": receipt.last_commit_sequence,
      "X-Particeps-Commit-Count": receipt.commit_count,
