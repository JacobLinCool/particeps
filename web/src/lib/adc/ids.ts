...
/* ---- the two key names ---------------------------------------------------------------------
 *
 * Domain separation, so the same key never derives one string under two roles. Twenty-one ASCII
 * bytes each, prefixed to the key before hashing. */
const SIGNER_DOMAIN = 'adc:signer-key-id:v1:';
const EXPORT_DOMAIN = 'adc:export-key-id:v1:';
