...
from adc_analysis.event_store import EventDatabase
from adc_analysis.models import (
    BootSession,
    EventProvenance,
    SamplingGroup,
...
from adc_analysis.reassembly import ReassemblyResult
from adc_analysis.sink import ParquetSink, _observations
...
class SinkTest(unittest.TestCase):
    def test_all_p2_collectors_materialize_typed_parquet_fixtures(self) -> None:
        ...
        result = ReassemblyResult((), _Events(events), {"format": "adc-quality-summary-v1"}, False)
    ...
    def test_parquet_batches_obey_the_estimated_byte_cap(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            destination = Path(temporary) / "dataset"
            with patch(
                "adc_analysis.sink.PARQUET_BATCH_MAX_ESTIMATED_BYTES", 1
            ):
                ParquetSink(CollectorCatalog(CATALOG)).write(result, destination)
