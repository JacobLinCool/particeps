...
from adc_analysis.jcs import canonicalize, parse
from adc_analysis.models import SourceObject
...
class InventoryTest(unittest.TestCase):
    def test_local_inventory_is_content_addressed_and_reloadable(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            source = root / "source"
            source.mkdir()
            (source / "b.adcexp").write_bytes(b"ADCEXP01-b")
            (source / "a.adcexp").write_bytes(b"ADCEXP01-a")
    ...
    def test_receiver_metadata_is_closed_world_and_checked(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            objects = inventory.ingest(
                [
                    S3BundleSource(
                        ...
                        client=FakeS3Client(
                            {
                                "prefix/b.adcexp": (data, metadata),
                                "prefix/a.adcexp": (data, metadata),
                            }
                        ),
                    )
                ]
            ...
            self.assertEqual(
                ["s3://bucket/prefix/a.adcexp", "s3://bucket/prefix/b.adcexp"],
    ...
    def test_inventory_manifest_cannot_redirect_a_cache_entry(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            source = root / "source.adcexp"
            source.write_bytes(b"ADCEXP01-content")
            inventory = CiphertextInventory(root / "workspace")
