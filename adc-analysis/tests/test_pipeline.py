...
from adc_analysis.encoding import base64url_decode
from adc_analysis.errors import ValidationError
from adc_analysis.inventory import CiphertextInventory
from adc_analysis.pipeline import AnalysisPipeline, load_private_keys
from adc_analysis.sink import ParquetSink
from adc_analysis.sources import LocalBundleSource, S3BundleSource
...
class PipelineTest(unittest.TestCase):
    ...
    def test_valid_bundle_materializes_typed_parquet_with_provenance(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            source = root / "source"
            source.mkdir()
            (source / "valid.adcexp").write_bytes(
            ...
            self.assertEqual("adc-parquet-dataset-v1", manifest["dataset_format"])
    ...
    def test_invalid_bundle_is_quarantined_without_partial_rows(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            encoded[-1] ^= 1
            (source / "invalid.adcexp").write_bytes(encoded)
            workspace = root / "workspace"
            CiphertextInventory(workspace).ingest([LocalBundleSource([source])])
            ...
            self.assertEqual(1, len(list((workspace / "quarantine").rglob("*.adcexp"))))
    ...
    def test_invalid_bundle_does_not_contribute_rows_when_valid_data_remains(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            invalid[-1] ^= 1
            (source / "valid.adcexp").write_bytes(valid)
            (source / "invalid.adcexp").write_bytes(invalid)
            workspace = root / "workspace"
            CiphertextInventory(workspace).ingest([LocalBundleSource([source])])
