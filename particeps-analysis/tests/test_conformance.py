...
from particeps_analysis.crypto import open_base
from particeps_analysis.encoding import base64url_decode, sha256_hex, uuid4_text
from particeps_analysis.errors import ValidationError
from particeps_analysis.jcs import canonical_decimal, canonicalize, exact_object, parse
...
class ProtocolConformanceTest(unittest.TestCase):
    ...
    def _verify_container(self, encoded: bytes):
        bundle = self.corpus["valid"]["bundle"]
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / "vector.partexp"
...
def _validate_signed_configuration(encoded: bytes, catalog: CollectorCatalog) -> None:
    if len(encoded) < 14 + 64 or encoded[:8] != b"PTCCFG01":
