...
from particeps_analysis.sources import S3BundleSource
...
class CliTest(unittest.TestCase):
    ...
    def test_one_inventory_snapshot_can_combine_local_and_receiver_sources(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            local = root / "manual.partexp"
            local.write_bytes(encoded)
            workspace = root / "workspace"
            receiver = S3BundleSource(
                "bucket",
                client=FakeS3Client({"automatic.partexp": (encoded, metadata)}),
            )
            with (
                patch("particeps_analysis.cli.S3BundleSource", return_value=receiver),
