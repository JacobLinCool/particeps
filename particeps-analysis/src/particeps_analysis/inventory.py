...
class CiphertextInventory:
    ...
    def ingest(self, sources: Iterable[BundleSource]) -> tuple[InventoryObject, ...]:
        ...
        document = {
            "format": "particeps-ciphertext-inventory-v1",
            "objects": [
                {
    ...
    def load(self) -> tuple[InventoryObject, ...]:
        ...
        if document["format"] != "particeps-ciphertext-inventory-v1" or not isinstance(
        ...
        for raw in document["objects"]:
            ...
            expected_relative = (
                Path("cache") / "objects" / digest[:2] / f"{digest}.partexp"
            )
    ...
    def _ingest_object(self, item) -> InventoryObject:
        ...
        count = 0
        try:
            fd, temporary_name = tempfile.mkstemp(prefix=".particeps-object-", dir=self.cache)
            ...
            destination = self.cache / sha256[:2] / f"{sha256}.partexp"
